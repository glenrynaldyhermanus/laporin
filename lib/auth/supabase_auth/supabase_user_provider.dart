...
class LaporinSupabaseUser extends BaseAuthUser {
  LaporinSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> laporinSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LaporinSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
